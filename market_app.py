...
portfolio = build_optimized_portfolio(market, PORTFOLIO_TICKERS, method="sharpe")
...
with tabs[2]:
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=portfolio.index.index, y=portfolio.index.values, mode="lines", line=dict(width=3, color="#0d6e6e"), name="Long-only Sharpe-maximizing portfolio"))
    fig.update_layout(title="Long-only Sharpe-maximizing portfolio performance", height=460, margin=dict(l=20, r=20, t=50, b=20), yaxis_title="Indexed to 100")
    st.plotly_chart(fig, width="stretch")
    ...
    st.caption(
        f"Portfolio uses a simple long-only Sharpe-maximizing allocation, re-estimated annually from trailing daily returns. "
